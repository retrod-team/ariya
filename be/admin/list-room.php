...
<?php include(SERVER_ADMIN_SCREEN_PATH.'sidebar.php') ?>
                    ...
                    <div class="col-auto my-auto">
                        <div class="h-100">
                            ...
                            <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                                ...
                                <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Manage room</li>
                            </ol>
                        </div>
                    </div>
                </div>
                                ...
                                <div class="card" style="padding: 25px 10px;">
                                    <div class="table table-responsive">
                                        <table class="table align-items-center mb-0 tableLine">
                                            ...
                                            <?php 
                                                $si = 0;
                                                $sql = mysqli_query($conDB, "select * from room where pId = '0'");
                                                if(mysqli_num_rows($sql) > 0){
                                                    while($row = mysqli_fetch_assoc($sql)){
                                                        ...
                                                        if($row['status'] == 1){
                                                            $status = "<a class='tableIcon status bg-gradient-success deactive' href='list-room.php?status=$id' data-bs-toggle='tooltip' data-bs-placement='top' data-bs-original-title='Deactive'><i class='far fa-eye'></i></a>";
                                                        }else{
                                                            $status = "<a class='tableIcon status bg-gradient-warning  active' href='list-room.php?status=$id' data-bs-toggle='tooltip' data-bs-placement='top' data-bs-original-title='Active'><i class='far fa-eye-slash'></i></a>";
                                                        }
                                                        $delete = "<a class='tableIcon delete bg-gradient-danger' href='list-room.php?delete=$id' data-bs-toggle='tooltip' data-bs-placement='top' data-bs-original-title='Delete'><i class='far fa-trash-alt'></i></a>";
                                                        $update = "<a class='tableIcon update bg-gradient-info' href='manage-room.php?update=$id' data-bs-toggle='tooltip' data-bs-placement='top' data-bs-original-title='Edit'><i class='far fa-edit'></i></a>";
  ...
  <script>
      ...
      $('.manageRoomLink').addClass('active');
      $('.roomLink').addClass('active');

        $attr_count = 1;
        $('.add_sub').on('click', function(){
            $attr_count ++;
            $html = '<div class="row" id="add_content_id' + $attr_count + '">';
            $html += '<div class="form_group col_4"><label for="title' + $attr_count + '">Title</label><input class="form_control" type="text" id="title' + $attr_count + '" name="title[]" placeholder="Enter Title."></div>';
            $html += '<div class="form_group col_3"><label for="roomPrice' + $attr_count + '">Room Price</label><input class="form_control" type="text" id="roomPrice' + $attr_count + '" name="roomPrice[]" placeholder="Enter Room Price."></div>';
            $html += '<div class="form_group col_5"><label for="roomImage' + $attr_count + '">Room Image</label><input class="form_control" type="file" id="roomImage' + $attr_count + '" name="roomImage[]"></div>';
            $html += '<div class="form_group col_2"><label for="noAdult' + $attr_count + '">No of Adult</label><input class="form_control" type="text" id="noAdult' + $attr_count + '" name="noAdult[]"></div>';
            $html += '<div class="form_group col_3"><label for="extraAdult' + $attr_count + '">Extra charge of Adult</label><input class="form_control" type="text" id="extraAdult' + $attr_count + '" name="extraAdult[]"></div>';
            $html += '<div class="form_group col_3"><label for="extraChild' + $attr_count + '">Extra charge of Child</label><input class="form_control" type="text" id="extraChild' + $attr_count + '" name="extraChild[]"></div>';
            $html += '<div class="remove_sub col_2 btn" data-id="' + $attr_count + '">Remove</div>';
            $html += '</div>';
        ...
        $(document).on('submit', '#loginForm', function(e){
			...
			$.ajax({
				...
				success : function(data){
					...
					// $arr = $.parseJSON(data); 
					// if ( $arr.type == 'success') {
					// 	$('.alert.success_box').slideDown('slow');
					// 	$('.alert.success_box span').html($arr.msg);
					// 	setTimeout(hide_time, 1500);
					// 	hotelIdGenerate();
					// 	if ($("#hotel_id").length == 0){
					// 		$('#loginForm').trigger("reset");
					// 	}
					// } else {
					// 	$('.alert.error_box').slideDown('slow');
					// 	$('.alert.error_box span').html($arr.msg);
					// 	setTimeout(hide_time, 1500);
					// }
