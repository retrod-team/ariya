(function ($) {
    ...
    // let site = 'https://aryapalace.retrod.in/';
    let site = 'https://localhost/niladri/be/';

    function userRoomCheck($rid, $rdid, $room) {
        var id = $rdid;
        var room_id = $rid;
        var room = $room;
        var url = site + 'admin/include/ajax/room.php';
        $.ajax({
            url: url,
            type: 'post',
    ...
    function roomCheckoutDateUpdate($key){
        var key = $key;
        $.ajax({
            url: site + "/admin/include/ajax/room_detail.php",
            type: 'post',
    ...
    $(document).on('submit', '#room_guest_select_form', function (e) {
        e.preventDefault();
        $.ajax({
            url: site + 'admin/include/ajax/room.php',
            type: 'post',
    ...
    $(document).on('click', '#pickup', function () {
        if ($(this).is(':checked')) {
            $.ajax({
                ...
                success: function (data) {
                    priceLoad();
                    $html = '<span>Pick & Drop</span><span>Rs <strong>' + data + '</strong></span>';
                    $('#pickupContent').html($html).css("display", "flex");
                }
            });
        } else {
            $.ajax({
    ...
    $(document).on('click', '#payAtHotel', function () {
        if ($(this).is(':checked')) {
            $.ajax({
                url: site + 'admin/include/ajax/room.php',
                type: 'post',
                data: { type: 'payAtHotel' },
                success: function (data) {
                    priceLoad();
                    $html = '<span>Total Payment </span><span>Rs <strong>0.00</strong></span>';
                    $('#payathotelContent').html($html).css("display", "flex");
                }
            });
        } else {
            $.ajax({
                url: site + 'admin/include/ajax/room.php',
                type: 'post',
                data: { type: 'removePayAtHotel' },
                success: function (data) {
                    priceLoad();
                    $('#payathotelContent').hide().html('');
                }
            });
        }
    });
    ...
    $(document).on('click', '.CheckNight', function (e) {
        ...
        var rid = $(this).data('rid');
        $.ajax({
            url: site + 'admin/include/ajax/otherDetail.php',
            type: 'post',
